...
public class LoginInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		String head="/Tmall_SSM_war_exploded";
		String[] noNeedAuthPage = new String[]{
				"/Tmall_SSM_war_exploded/",
				head+"/home.do",
				head+"/classSearchList.html",
				"/sortProduct",
				"/showProduct",
				"/loginPage",
				"/login",
				head+"/registerPage.html",
				head+"/register.do",
				"/registerSuccessPage",
				"/test",
				"/checkLogin",
				"/admin",
				"/Tmall_SSM_war_exploded/loginPage.html",
				head+"/GetUser.do",
				head+"/login.do",
				head+"/logout.do"
		...
		if (!Arrays.asList(noNeedAuthPage).contains(uri)) {
			...
			if (null == user) {
				...
				response.sendRedirect(head+"/loginPage.html");
				return false;
			}
		}
		return true;
